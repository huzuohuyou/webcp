	...
	function DBOption(){
		$.ajax({
			url: url,
	 	    ...
	 	    async: async,
	 	    data: {op : operate,
	 	    	zxksbm_ajax : zxksbm_ajax,
	 	    	zxksmc_ajax : encodeURI(zxksmc_ajax),
	 	    	zxkspy_ajax : encodeURI(zxkspy_ajax),
	 	    	zxkswb_ajax : encodeURI(zxkswb_ajax),
	 	    	bdksbm_ajax :bdksbm_ajax,
	 	    	bdksmc_ajax : encodeURI(bdksmc_ajax),
	 	    	bdkspy_ajax : encodeURI(bdkspy_ajax),
	 	    	bdkswb_ajax : encodeURI(bdkswb_ajax),
	 	    	zcl_ajax : encodeURI(zcl_ajax),
	 	    	ysj_ajax : encodeURI(ysj_ajax),
	 	    	yxj_add_ajax : encodeURI(yxj_add_ajax),
	 	    	yxj_del_ajax : encodeURI(yxj_del_ajax),
	 	    	delStr : delStr,
				pageNo : pageNo,
	 	    	execute:false
	 	    	},
...
//左侧只允许点击一次
	...
	function tianjia(){
		...
		if(ycl_array_zx.length==0||zcl_ajax==""){
			alert("你的选择不符合要求");
		}else{
			var zcl_xz$=$("tr[name^='zuocelan'][id='"+zcl_ajax+"']");
