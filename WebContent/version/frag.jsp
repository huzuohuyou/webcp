    ...
    <table width="100%"  border="0" cellpadding="0" cellspacing="1" bgcolor="#a8c7ce" class="frag-3" >
       <thead>
		...
		<tr>
          ...
          <th width="28%" align="center" height="19" bgcolor="d3eaef" class="STYLE10">医嘱项目</th>
        ...
        <tbody id="lcp_node_order_tbody">
          ...
          String sql="select CP_NODE_ORDER_ID,CP_NODE_ORDER_TEXT,NEED_ITEM,AUTO_ITEM,ORDER_KIND from lcp_node_order_point  "
					+  " where SYS_IS_DEL=0 and  cp_id="+cp_id+" and cp_node_id="+cp_node_id+" and HOSPITAL_ID="+HOSPITALID;
 			String itemSql="select cp_node_order_item_id,cp_node_order_text,need_item,auto_item,order_no from lcp_node_order_item "
 						+"where sys_is_del=0 and cp_id="+cp_id+" and cp_node_id="+cp_node_id+" and HOSPITAL_ID="+HOSPITALID+" and CP_NODE_order_ID=";
 	     ...
 	     %>
         <tr bgcolor="#FFFFFF" onMouseOver="changeColor(this)" onMouseOut="recoverColor(this)" class="STYLE19"  id="node-<%= cpNodeOrderId%>">
			<td align="center"></td>
           	<td  colspan="2" align="left" class="STYLE10"><%=cpNodeOrderText %></td>
          	<td  align="center" class="STYLE10"><%=needItem %></td>
          	<td align="center"  class="STYLE10"><%=orderType %></td>
         ...
         <%if(ds1.FunGetDataAsStringById(0,0)!=""){
			for (int j = 0; j < ds1.FunGetRowCount(); j++) {
				...
				if("0".equals(auto)){
					auto="自动";
				}else if("1".equals(auto)){
					auto="手动";
				}
				if("1".equals(need)){need="必做";
				}else { need="可做";}
				String matchSql="select local_code,match_type from lcp_match_order where sys_is_del=0 and order_code='"+orderNo+"'"+" and HOSPITAL_ID="+HOSPITALID;
				DataSetClass matchDs=db.FunGetDataSetBySQL(matchSql); 
				String localCodeTemp="";
				if(matchDs.FunGetDataAsStringById(0,0)!=""){
					for(int s=0;s<matchDs.FunGetRowCount();s++){
		...
		<tr class="child-of-node-<%= cpNodeOrderId%>" bgcolor="#FFFFFF" onMouseOver="changeColor(this)" onMouseOut="recoverColor(this)"   id="tr<%=cpNodeOrderItemId%>">
		  <td><div align="center"></div></td>
          <td align="left"   class="STYLE10" >&nbsp;&nbsp;&nbsp;&nbsp;<%=orderText %></td>
          <td align="center" class="STYLE10"><%=orderNo %></td>
          <td align="center" class="STYLE10"><%=need %></td>
