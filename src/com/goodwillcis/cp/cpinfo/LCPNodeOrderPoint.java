...
public class LCPNodeOrderPoint implements CustomDataSet {
	...
	CP_NODE_ORDER_TEXT	VARCHAR2(50)	Y			路径节点医嘱项内容
	CONTINUE_ITEM	NUMBER(4)	Y			延续项目（0不延续，1延续）
	CONTINUE_CP_NODE_ID	NUMBER(5)	Y			延续路径节点编号
	CONTINUE_ORDER_ID	NUMBER(5)	Y			延续医嘱项编号
	...
	metaData = new String[] { "CP_NODE_ID", "CP_NODE_ORDER_ID","CP_NODE_ORDER_TEXT","ORDER_KIND" };
	data = new ArrayList<Object[]>();
	DatabaseClass db = LcpUtil.getDatabaseClass();
	String sql = "select CP_NODE_ID, CP_NODE_ORDER_ID, CP_NODE_ORDER_TEXT, ORDER_KIND from LCP_NODE_ORDER_POINT where CP_ID =  "+ cpID+" order by CP_NODE_ID, ORDER_KIND, CP_NODE_ORDER_ID" ;
	...
	for (int i = 0; i < count; i++) {
	    ...
	    String cp_node_doctor_text = sd.FunGetDataAsStringByColName(i, "CP_NODE_ORDER_TEXT");
	    String ORDER_KIND = sd.FunGetDataAsStringByColName(i, "ORDER_KIND");
	    data.add(new Object[] { cp_node_id, cp_node_doctor_id,cp_node_doctor_text,ORDER_KIND });
	}
