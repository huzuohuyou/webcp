...
import com.goodwillcis.lcp.service.match.impl.MatchNurseImpl;

public class MatchNurseServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	MatchNurse match=new MatchNurseImpl();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("getAllTable".equals(operate)){
			...
			String sql="    SELECT A.* " +
				"   FROM DCP_DICT_NURSE A" +
				"   ORDER BY A.NURSE_CODE";
			...
			String localTable=matchlocalTable.funGetTable("select * from LCP_LOCAL_NURSE t","NURSE_CODE","NURSE_name","input_code_py","input_code_wb");
		...
		if("pipeiutil".equals(operate)){
			...
			if(zcl_ajax!=""){
				MatchLocal matchlocalTable=new MatchLocalImpl();
				localTable=matchlocalTable.funGetTable("select * from LCP_LOCAL_NURSE t","NURSE_CODE","NURSE_name","input_code_py","input_code_wb");
				youxiajiaoHtml=match.funCreateMatchTable(zcl_ajax);
				String sql="SELECT * FROM DCP_DICT_NURSE T WHERE T.NURSE_CODE='"+zcl_ajax+"'";
				DataSet _dataset=new DataSet(); 
				_dataset.funSetDataSetBySql(sql);
				_code=_dataset.funGetFieldByCol(0, "NURSE_CODE");
				_mingcheng=_dataset.funGetFieldByCol(0, "NURSE_NAME");
		...
		if("changeTable".equals(operate)){
			...
			if(zxksbm_ajax!=""&&zxksbm_ajax!=null){
				sql=sql+" and (A.NURSE_CODE like '%"+zxksbm_ajax+"%' or A.NURSE_CODE like '%"+zxksbm_ajax.toUpperCase()+"%' or A.NURSE_CODE like '%"+zxksbm_ajax.toLowerCase()+"%')";
			}
			if(zxksmc_ajax!=""&&zxksmc_ajax!=null){
				if(zxksmc_ajax.indexOf("_")>0){
					zxksmc_ajax=zxksmc_ajax.replace("_", "/_");
					zxksmc_ajax=zxksmc_ajax+"%' escape '/'";
					sql=sql+" and  A.NURSE_name like '%"+zxksmc_ajax+" ";
				}else{
					sql=sql+" and  A.NURSE_name like '%"+zxksmc_ajax+"%' ";
				}
			}
		...
		if("ycchaxun".equals(operate)){//右侧查询
			...
			String sql="select * from LCP_LOCAL_NURSE a where 1=1 ";
			if(bdksbm_ajax!=""&&bdksbm_ajax!=null){
				sql=sql+" and (A.NURSE_CODE like '%"+bdksbm_ajax+"%' or A.NURSE_CODE like '%"+bdksbm_ajax.toUpperCase()+"%' or A.NURSE_CODE like '%"+bdksbm_ajax.toLowerCase()+"%')";
			}
			if(bdksmc_ajax!=""&&bdksmc_ajax!=null){
				if(bdksmc_ajax.indexOf("_")>=0||bdksmc_ajax.indexOf("%")>=0){
					...
					sql=sql+" and  A.NURSE_name like '%"+bdksmc_ajax+" ";
				}else{
					sql=sql+" and  A.NURSE_name like '%"+bdksmc_ajax+"%' ";
				}
			}
			...
			//System.out.println(sql);
			MatchLocal matchlocalTable=new MatchLocalImpl();
			String localTable=matchlocalTable.funGetTable(sql,"NURSE_CODE","NURSE_name","input_code_py","input_code_wb");
			response.getWriter().println("[{\"result\":\"OK\"," +
					"\"method\":\"ycchaxun\", " +
					"\"localTable\":\""+localTable+"\"}]");
		}
