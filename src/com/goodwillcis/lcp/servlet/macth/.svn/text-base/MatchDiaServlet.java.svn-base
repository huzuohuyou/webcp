...
import com.goodwillcis.lcp.service.match.impl.MatchDiaImpl;
import com.goodwillcis.lcp.service.match.impl.MatchLocalImpl;


public class MatchDiaServlet extends HttpServlet {
	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	MatchDia match=new MatchDiaImpl();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		if("getAllTable".equals(operate)){
			...
			String sql="    SELECT A.* " +
				"   FROM DCP_DICT_DIAGNOSIS A" +
				"   ORDER BY A.DIAGNOSIS_CODE";
			...
			String localTable=matchlocalTable.funGetTable("select * from lcp_local_diagnosis t","DIAGNOSIS_CODE","DIAGNOSIS_name","input_code_py","input_code_wb");
		...
		if("changeTable".equals(operate)){
			...
			if(zxksbm_ajax!=""&&zxksbm_ajax!=null){
				sql=sql+" and (A.DIAGNOSIS_CODE like '%"+zxksbm_ajax+"%' or A.DIAGNOSIS_CODE like '%"+zxksbm_ajax.toUpperCase()+"%' or A.DIAGNOSIS_CODE like '%"+zxksbm_ajax.toLowerCase()+"%')";
			}
		...
		if("ycchaxun".equals(operate)){//右侧查询
			...
			if(bdksbm_ajax!=""&&bdksbm_ajax!=null){
				sql=sql+" and (A.DIAGNOSIS_CODE like '%"+bdksbm_ajax+"%' or A.DIAGNOSIS_CODE like '%"+bdksbm_ajax.toUpperCase()+"%' or A.DIAGNOSIS_CODE like '%"+bdksbm_ajax.toLowerCase()+"%')";
			}
			...
			MatchLocal matchlocalTable=new MatchLocalImpl();
			String localTable=matchlocalTable.funGetTable(sql,"DIAGNOSIS_CODE","DIAGNOSIS_name","input_code_py","input_code_wb");
			response.getWriter().println("[{\"result\":\"OK\"," +
					"\"method\":\"ycchaxun\", " +
					"\"localTable\":\""+localTable+"\"}]");
		}
		...
		if("pipeiutil".equals(operate)){
			...
			if(zcl_ajax!=""){
				MatchLocal matchlocalTable=new MatchLocalImpl();
				localTable=matchlocalTable.funGetTable("select * from lcp_local_diagnosis t","DIAGNOSIS_CODE","DIAGNOSIS_name","input_code_py","input_code_wb");
				youxiajiaoHtml=match.funCreateMatchTable(zcl_ajax);
