...
import com.goodwillcis.lcp.service.match.impl.MatchDoctorImpl;
import com.goodwillcis.lcp.service.match.impl.MatchLocalImpl;


public class MatchDoctorServlet extends HttpServlet {

	/**
	 * 
	 */
	private static final long serialVersionUID = 1L;
	MatchDoctor match=new MatchDoctorImpl();
	...
	//		//System.out.println(sql);
			...
			String localTable=matchlocalTable.funGetTable("select * from lcp_local_doctor t","DOCTOR_CODE","DOCTOR_name","input_code_py","input_code_wb");
		...
		if("pipeiutil".equals(operate)){
			...
			if(zcl_ajax!=""){
				MatchLocal matchlocalTable=new MatchLocalImpl();
				localTable=matchlocalTable.funGetTable("select * from lcp_local_doctor t","DOCTOR_CODE","DOCTOR_name","input_code_py","input_code_wb");
				youxiajiaoHtml=match.funCreateMatchTable(zcl_ajax);
				String sql="SELECT * FROM DCP_DICT_DOCTOR T WHERE T.DOCTOR_CODE='"+zcl_ajax+"'";
				DataSet _dataset=new DataSet(); 
				_dataset.funSetDataSetBySql(sql);
				_code=_dataset.funGetFieldByCol(0, "DOCTOR_CODE");
				_mingcheng=_dataset.funGetFieldByCol(0, "DOCTOR_NAME");
	...
	//		//System.out.println(zxksbm_ajax+"  "+zxksmc_ajax+"  "+zxkspy_ajax+"  "+zxkswb_ajax);			
			...
			if(zxksbm_ajax!=""&&zxksbm_ajax!=null){
				sql=sql+" and (A.DOCTOR_CODE like '%"+zxksbm_ajax+"%' or A.DOCTOR_CODE like '%"+zxksbm_ajax.toUpperCase()+"%' or A.DOCTOR_CODE like '%"+zxksbm_ajax.toLowerCase()+"%')";
			}
			if(zxksmc_ajax!=""&&zxksmc_ajax!=null){
				if(zxksmc_ajax.indexOf("_")>=0||zxksmc_ajax.indexOf("%")>=0){
					...
					sql=sql+" and  A.DOCTOR_name like '%"+zxksmc_ajax+" ";
				}else{
					sql=sql+" and  A.DOCTOR_name like '%"+zxksmc_ajax+"%' ";
				}
			}
			...
			sql=sql+"  ORDER BY A.DOCTOR_CODE";
	//		//System.out.println(sql);
			String pageNo_js=request.getParameter("pageNo");
			int pageNo=1;
			try {
	...
	//		//System.out.println(bdksbm_ajax+"  "+bdksmc_ajax+"  "+bdkspy_ajax+"  "+bdkswb_ajax);
			String sql="select * from lcp_local_doctor a where 1=1 ";
			if(bdksbm_ajax!=""&&bdksbm_ajax!=null){
				sql=sql+" and (DOCTOR_CODE like '%"+bdksbm_ajax+"%' or DOCTOR_CODE like '%"+bdksbm_ajax.toUpperCase()+"%' or DOCTOR_CODE like '%"+bdksbm_ajax.toLowerCase()+"%')";
			}
			if(bdksmc_ajax!=""&&bdksmc_ajax!=null){
				if(bdksmc_ajax.indexOf("_")>=0||bdksmc_ajax.indexOf("%")>=0){
					...
					sql=sql+" and  A.DOCTOR_name like '%"+bdksmc_ajax+" ";
				}else{
					sql=sql+" and  A.DOCTOR_name like '%"+bdksmc_ajax+"%' ";
				}
			}
	...
	//		//System.out.println(sql);
			MatchLocal matchlocalTable=new MatchLocalImpl();
			String localTable=matchlocalTable.funGetTable(sql,"DOCTOR_CODE","DOCTOR_name","input_code_py","input_code_wb");
			response.getWriter().println("[{\"result\":\"OK\"," +
					"\"method\":\"ycchaxun\", " +
					"\"localTable\":\""+localTable+"\"}]");
		}
