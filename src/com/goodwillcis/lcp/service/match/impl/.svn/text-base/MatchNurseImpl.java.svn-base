...
// 文件名： MatchNurseImpl.java
// 文件功能描述： 护理匹配表调用的各种方法实现类
// 创建人：刘植鑫 
// 创建日期：2011/07/26
// * 
...
import com.goodwillcis.lcp.service.match.MatchNurse;
import com.goodwillcis.lcp.util.CommonUtil;
import com.goodwillcis.lcp.util.LcpUtil;

public class MatchNurseImpl implements MatchNurse {
	...
	public String funCreateTable(String sql, int start, int end) {
		...
		for(int i=0;i<row;i++){
			String code=dataSet.funGetFieldByCol(i, "NURSE_CODE");
			String name=dataSet.funGetFieldByCol(i, "NURSE_NAME");
	...
	public String funCreateMatchTable(String code) {
		// TODO Auto-generated method stub
		String sql=" SELECT T.*,NURSE_NAME FROM LCP_MATCH_NURSE T,DCP_DICT_NURSE A  WHERE T.NURSE_CODE=A.NURSE_CODE AND T.NURSE_CODE='"+code+"'";
		DataSet dataSet=new DataSet();
	...
	//	System.out.println(localCodes);
		...
		if(localCodes!=""){
			String localSql="SELECT * FROM LCP_LOCAL_NURSE T WHERE NURSE_CODE IN("+localCodes+")";
			localList=localTable.funGetCoreList(localSql, "NURSE_CODE", "NURSE_NAME", "INPUT_CODE_PY", "INPUT_CODE_WB");
		}
		//用于显示的表格信息
		String table="";
		...
		if(dataSet_row>0){
			for(int ii=0;ii<dataSet_row;ii++){
				String zxbm=dataSet.funGetFieldByCol(0, "NURSE_CODE");
				String zxmc=dataSet.funGetFieldByCol(0, "NURSE_NAME");
...
//		System.out.println(table);
	...
	public int funGetCountByCode(String code) {
		// TODO Auto-generated method stub
		String sql="SELECT COUNT(*)HANG FROM LCP_MATCH_NURSE Tt WHERE NURSE_CODE='"+code+"'";
		DataSet dataSet=new DataSet();
		dataSet.funSetDataSetBySql(sql);
		int aa=Integer.parseInt(dataSet.funGetFieldByCol(0, "HANG"));
		return aa;
	...
	public int funPiPeiInsert(String IDS) {
		...
		for(int i=0;i<len;i++){
			...
			DataSet dataSet=new DataSet();
			String linshiSql="SELECT COUNT(*)HANG FROM LCP_MATCH_NURSE T WHERE T.LOCAL_CODE='"+localCode+"'";
			...
			sql=sql+"INSERT INTO LCP_MATCH_NURSE (HOSPITAL_ID,NURSE_CODE,LOCAL_CODE,MATCH_TYPE,SYS_IS_DEL,SYS_LAST_UPDATE) " +
			"values("+hospitalID+",'"+coreCode+"','"+localCode+"',0,0,"+time+")\r\n";
			sql= sql+"update dcp_dict_NURSE T set t.match_flag=1 where NURSE_CODE='"+coreCode+"'\r\n";
			sql= sql+"update LCP_LOCAL_NURSE T set t.match_flag=1 where NURSE_CODE='"+localCode+"'\r\n";
		}
	...
	public int funUpdateYDD(String code) {
		...
		String time=CommonUtil.getOracleToDate();
		String sql="UPDATE LCP_MATCH_NURSE T SET T.MATCH_TYPE="+type+" ,T.SYS_LAST_UPDATE="+time+" WHERE NURSE_CODE='"+code+"'";
	...
	public int funDelMatch(String IDS) {
		...
		for(int i=0;i<len;i++){
			...
			coreCode=coreCodeAndLocalCodeArr[0];
			String localCode=coreCodeAndLocalCodeArr[1];
			sql=sql+"DELETE LCP_MATCH_NURSE T WHERE NURSE_CODE='"+coreCode+"' AND T.LOCAL_CODE='"+localCode+"'\r\n";
			sql=sql+"update LCP_LOCAL_NURSE t set t.match_flag=0 where NURSE_CODE='"+localCode+"'\r\n";
		}
//		System.out.println(sql);
		DatabaseClass database=LcpUtil.getDatabaseClass();
		int aa=-1;
		...
		String dictSql="select count(*) hang from dcp_dict_NURSE a, LCP_MATCH_NURSE b where A.NURSE_CODE=b.NURSE_CODE and a.NURSE_CODE='"+coreCode+"'";
		...
		if(dict_match_row==0){
			sql="update dcp_dict_NURSE t set t.match_flag=0 where NURSE_CODE='"+coreCode+"'\r\n";
		}
		System.out.println(sql);
	...
	public int funFindIsUsedBeforeDel(String IDS) {
		...
		sql="SELECT COUNT(*)HANG FROM LCP_PATIENT_LOG_NURSE T WHERE T.LOCAL_CODE IN ("+localCode+") AND NURSE_NO ='"+coreCode+"'";
		DataSet dataSet=new DataSet();
		dataSet.funSetDataSetBySql(sql);
