...
// 文件名： MatchDoctorImpl.java
// 文件功能描述： 诊疗匹配表调用的各种方法实现类
// 创建人：刘植鑫 
// 创建日期：2011/07/26
//  * 
...
import com.goodwillcis.lcp.service.match.MatchDoctor;
...
public class MatchDoctorImpl implements MatchDoctor {
	...
	public String funCreateTable(String sql, int start, int end) {
		...
		for(int i=0;i<row;i++){
			String code=dataSet.funGetFieldByCol(i, "DOCTOR_CODE");
			String name=dataSet.funGetFieldByCol(i, "DOCTOR_NAME");
	...
	public String funCreateMatchTable(String code) {
		// TODO Auto-generated method stub
		String sql=" SELECT T.*,DOCTOR_NAME FROM LCP_MATCH_DOCTOR T,DCP_DICT_DOCTOR A  WHERE T.DOCTOR_CODE=A.DOCTOR_CODE AND T.DOCTOR_CODE='"+code+"'";
		DataSet dataSet=new DataSet();
...
//		System.out.println(localCodes);
		...
		if(localCodes!=""){
			String localSql="SELECT * FROM lcp_local_doctor T WHERE DOCTOR_CODE IN("+localCodes+")";
			localList=localTable.funGetCoreList(localSql, "DOCTOR_CODE", "DOCTOR_NAME", "INPUT_CODE_PY", "INPUT_CODE_WB");
		}
		//用于显示的表格信息
		String table="";
		...
		if(dataSet_row>0){
			for(int ii=0;ii<dataSet_row;ii++){
				String zxbm=dataSet.funGetFieldByCol(0, "DOCTOR_CODE");
				String zxmc=dataSet.funGetFieldByCol(0, "DOCTOR_NAME");
	...
	public int funGetCountByCode(String code) {
		// TODO Auto-generated method stub
		String sql="SELECT COUNT(*)HANG FROM LCP_MATCH_DOCTOR Tt WHERE DOCTOR_CODE='"+code+"'";
		DataSet dataSet=new DataSet();
		dataSet.funSetDataSetBySql(sql);
		int aa=Integer.parseInt(dataSet.funGetFieldByCol(0, "HANG"));
		return aa;
	...
	public int funPiPeiInsert(String IDS) {
		...
		for(int i=0;i<len;i++){
			...
			DataSet dataSet=new DataSet();
			String linshiSql="SELECT COUNT(*)HANG FROM LCP_MATCH_DOCTOR T WHERE T.LOCAL_CODE='"+localCode+"'";
			...
			sql=sql+"INSERT INTO LCP_MATCH_DOCTOR (HOSPITAL_ID,DOCTOR_CODE,LOCAL_CODE,MATCH_TYPE,SYS_IS_DEL,SYS_LAST_UPDATE) " +
			"values("+hospitalID+",'"+coreCode+"','"+localCode+"',0,0,"+time+")\r\n";
			sql= sql+"update dcp_dict_DOCTOR t set t.match_flag=1 where DOCTOR_CODE='"+coreCode+"'\r\n";
			sql= sql+"update LCP_LOCAL_DOCTOR t set t.match_flag=1 where DOCTOR_CODE='"+localCode+"'\r\n";
		}
...
//		System.out.println(sql);
	...
	public int funUpdateYDD(String code) {
		...
		String time=CommonUtil.getOracleToDate();
		String sql="UPDATE LCP_MATCH_DOCTOR T SET T.MATCH_TYPE="+type+" ,T.SYS_LAST_UPDATE="+time+" WHERE DOCTOR_CODE='"+code+"'";
	...
	public int funDelMatch(String IDS) {
		...
		for(int i=0;i<len;i++){
			...
			coreCode=coreCodeAndLocalCodeArr[0];
			String localCode=coreCodeAndLocalCodeArr[1];
			sql=sql+"DELETE LCP_MATCH_DOCTOR T WHERE DOCTOR_CODE='"+coreCode+"' AND T.LOCAL_CODE='"+localCode+"'\r\n";
			sql=sql+"update LCP_LOCAL_DOCTOR t set t.match_flag=0 where DOCTOR_CODE='"+localCode+"'\r\n";
		}
	//	System.out.println(sql);
		DatabaseClass database=LcpUtil.getDatabaseClass();
		int aa=-1;
		...
		String dictSql="select count(*) hang from dcp_dict_DOCTOR a, LCP_MATCH_DOCTOR b where A.DOCTOR_CODE=b.DOCTOR_CODE and a.DOCTOR_CODE='"+coreCode+"'";
		...
		if(dict_match_row==0){
			sql="update dcp_dict_DOCTOR t set t.match_flag=0 where DOCTOR_CODE='"+coreCode+"'\r\n";
		}
		System.out.println(sql);
	...
	public int funFindIsUsedBeforeDel(String IDS) {
		...
		sql="SELECT COUNT(*)HANG FROM LCP_PATIENT_LOG_DOCTOR T WHERE T.LOCAL_CODE IN("+localCode+") AND DOCTOR_NO='"+coreCode+"'";
		DataSet dataSet=new DataSet();
		dataSet.funSetDataSetBySql(sql);
